...
export function MarketHero({ stats, onRunScan, onOpenJournal }: MarketHeroProps) {
  return (
    <section id="hero" className="relative overflow-hidden bg-white pt-16 sm:pt-28 md:pt-36 lg:pt-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center">
          {/* Market regime pill tag */}
          <div className="inline-flex items-center rounded-full bg-slate-100/90 p-1 mb-6 sm:mb-10 border border-slate-200/80 shadow-none">
            <span className="rounded-full bg-[#2952FF] text-white text-xs sm:text-sm font-semibold px-3 sm:px-4 py-1 sm:py-1.5 shadow-sm">
              {stats.regime}
            </span>
            <span className="text-xs sm:text-sm font-medium text-slate-700 px-3 sm:px-4 py-1 sm:py-1.5">
              {stats.sentimentTag}
            </span>
          </div>

          {/* Main bold headline */}
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-[76px] font-extrabold tracking-tight leading-[1.1] sm:leading-[1.05] text-slate-900 max-w-4xl">
            What deserves your attention right now
          </h1>

          {/* Subtitle with generous breathing room */}
          <p className="mt-6 sm:mt-10 text-slate-500 text-base sm:text-xl md:text-2xl max-w-2xl font-normal leading-relaxed px-2">
          ...
          <div className="mt-8 sm:mt-14 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-5 w-full sm:w-auto px-4 sm:px-0">
            ...
            <Button
              ...
              className="w-full sm:w-auto rounded-full border-slate-300 text-slate-900 hover:bg-slate-50 font-medium px-8 py-3.5 text-base transition-all h-auto"
            >
              Open journal
            </Button>
          </div>
          ...
          <div className="mt-12 sm:mt-20 grid grid-cols-2 sm:flex sm:flex-wrap items-center justify-center gap-4 sm:gap-x-14 sm:gap-y-4 text-xs sm:text-base text-slate-500 w-full sm:w-auto">
            <div className="flex flex-col sm:flex-row items-center sm:gap-2 p-2 sm:p-0 rounded-xl bg-slate-50 sm:bg-transparent border border-slate-100 sm:border-0">
              <span className="text-slate-400 text-xs sm:text-sm">Fear &amp; Greed</span>
              <div className="flex items-center gap-1">
                <span className="text-slate-900 font-semibold">{stats.fearAndGreed}</span>
                <span className="text-[11px] sm:text-xs text-slate-400 font-normal">({stats.fearAndGreedLabel})</span>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row items-center sm:gap-2 p-2 sm:p-0 rounded-xl bg-slate-50 sm:bg-transparent border border-slate-100 sm:border-0">
              <span className="text-slate-400 text-xs sm:text-sm">BTC dominance</span>
              <span className="text-slate-900 font-semibold">{stats.btcDominance}%</span>
            </div>

            <div className="flex flex-col sm:flex-row items-center sm:gap-2 p-2 sm:p-0 rounded-xl bg-slate-50 sm:bg-transparent border border-slate-100 sm:border-0">
              <span className="text-slate-400 text-xs sm:text-sm">Breadth</span>
              <span className="text-emerald-600 font-semibold capitalize">{stats.breadth}</span>
            </div>

            <div className="flex flex-col sm:flex-row items-center sm:gap-2 p-2 sm:p-0 rounded-xl bg-slate-50 sm:bg-transparent border border-slate-100 sm:border-0">
              <span className="text-slate-400 text-xs sm:text-sm">Volatility</span>
